...
class BenchmarkDemo : public CommonRigidBodyMTBase
{
	...
	btAlignedObjectArray<class RagDoll2*> m_ragdolls;
...
public:
	RagDoll2(btDynamicsWorld* ownerWorld, const btVector3& positionOffset, btScalar scale)
...
void BenchmarkDemo::createTest3()
{
	...
	while (size)
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			RagDoll2* ragDoll = new RagDoll2(m_dynamicsWorld, pos, scale);
