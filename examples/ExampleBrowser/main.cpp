...
int main(int argc, char* argv[])
{
...

	{
		...
		//Number of elapsed frames since the start of an iteration
		int frames = 0;
		iterations = 0;
		clock.reset();
		if (init)
		{
			exampleBrowser->getCurrentExample()->exitPhysics();
			exampleBrowser->demoSel(102); //102 is the index of our demo
			do
			{
				if (frames > 1000) 
				{
					frames = 0;
					iterations++;
					if (iterations > 22)//Number of defined trials
					{
					...
					exampleBrowser->getCurrentExample()->exitPhysics();
					exampleBrowser->demoSel(102);
					
				}
				float deltaTimeInSeconds = clock.getTimeMicroseconds() / 1000000.f;
				if (deltaTimeInSeconds > 0.5)
				{
					deltaTimeInSeconds = 0.5;
				}
				...
				{
					clock.reset();
					deltaTimeInSeconds = 0.005; // Fixed simulation timestep
